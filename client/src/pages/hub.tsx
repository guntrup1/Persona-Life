...
import {
  ...
  FileText, TrendingUp, ChevronDown, Star, X, ChevronLeft, ChevronRight,
} from "lucide-react";
...
const WELCOME_KEY = "tp_welcome_seen_v1";
    ...
    body: (
      <div className="space-y-3">
        ...
        <div className="mt-4 p-3 border border-primary/20 bg-primary/5 text-xs font-mono text-primary/80 leading-relaxed">
          ⚠️ Приложение в режиме <strong>Беты</strong>. Могут быть мелкие недочёты — пиши в обратную связь, я всё исправлю.
        </div>
        <div className="p-3 border border-yellow-500/20 bg-yellow-500/5 text-xs font-mono text-yellow-400/80 leading-relaxed">
          🐢 Сервер на бесплатном тарифе Render + MongoDB Atlas — первый отклик может быть чуть медленнее. Это нормально.
        </div>
      </div>
    ),
  },
  {
    ...
    body: (
      <div className="space-y-2">
        <p className="text-sm text-muted-foreground leading-relaxed">
          <span className="text-foreground font-semibold">Главная</span> — дашборд дня. Персонаж, часы, задачи, прогресс и заметки.
        </p>
        <div className="flex gap-3 items-start"><span className="text-primary text-xs mt-0.5 shrink-0">✅</span><p className="text-xs text-muted-foreground leading-relaxed"><span className="text-foreground">Задачи</span> — выполняй и получай XP. Стрик растёт каждый день.</p></div>
        <div className="flex gap-3 items-start"><span className="text-primary text-xs mt-0.5 shrink-0">⭐</span><p className="text-xs text-muted-foreground leading-relaxed"><span className="text-foreground">Цели</span> — недельные и месячные цели, разбитые на шаги.</p></div>
        <div className="flex gap-3 items-start"><span className="text-primary text-xs mt-0.5 shrink-0">📝</span><p className="text-xs text-muted-foreground leading-relaxed"><span className="text-foreground">Заметки дня</span> — мысли и наблюдения прямо на главной.</p></div>
      </div>
    ),
  },
  {
    ...
    body: (
      <div className="space-y-2">
        <div className="flex gap-3 items-start"><span className="text-primary text-xs mt-0.5 shrink-0">📈</span><p className="text-xs text-muted-foreground leading-relaxed"><span className="text-foreground">Трейдинг</span> — дневник сделок. Записывай входы, выходы, мысли по рынку.</p></div>
        <div className="flex gap-3 items-start"><span className="text-primary text-xs mt-0.5 shrink-0">📰</span><p className="text-xs text-muted-foreground leading-relaxed"><span className="text-foreground">Новости</span> — экономический календарь. High-impact события выделены красным.</p></div>
        <div className="flex gap-3 items-start"><span className="text-primary text-xs mt-0.5 shrink-0">📊</span><p className="text-xs text-muted-foreground leading-relaxed"><span className="text-foreground">Статистика</span> — графики продуктивности, история XP, стрики.</p></div>
      </div>
    ),
  },
  {
    ...
    body: (
      <div className="space-y-2">
        <div className="flex gap-3 items-start"><span className="text-primary text-xs mt-0.5 shrink-0">⏱</span><p className="text-xs text-muted-foreground leading-relaxed"><span className="text-foreground">Фокус</span> — помодоро-таймер. Время сессий записывается в статистику.</p></div>
        <div className="flex gap-3 items-start"><span className="text-primary text-xs mt-0.5 shrink-0">💡</span><p className="text-xs text-muted-foreground leading-relaxed"><span className="text-foreground">Идеи</span> — быстро сохраняй инсайты во время работы.</p></div>
        <div className="flex gap-3 items-start"><span className="text-primary text-xs mt-0.5 shrink-0">⚙️</span><p className="text-xs text-muted-foreground leading-relaxed"><span className="text-foreground">Настройки</span> — часовой пояс и торговые сессии.</p></div>
        <div className="flex gap-3 items-start"><span className="text-primary text-xs mt-0.5 shrink-0">💬</span><p className="text-xs text-muted-foreground leading-relaxed"><span className="text-foreground">Обратная связь</span> — кнопка внизу левого меню. Пиши баги и идеи.</p></div>
      </div>
    ),
  },
];
...
function WelcomePopup() {
  ...
  const goTo = (next: number, d: "left" | "right") => {
    ...
    setTimeout(() => { setSlide(next); setAnimating(false); }, 180);
  };

  const prev = () => slide > 0 && goTo(slide - 1, "left");
  const next = () => slide < welcomeSlides.length - 1 ? goTo(slide + 1, "right") : close();
  ...
  return (
    <>
      <div className="fixed inset-0 z-[300] bg-black/70" style={{ backdropFilter: "blur(6px)" }} onClick={close} />
      <div className="fixed inset-0 z-[301] flex items-center justify-center p-4 pointer-events-none">
        <div
          className="w-full max-w-md bg-card border border-border relative pointer-events-auto"
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-border">
            ...
            <button onClick={close} style={{ clipPath: "none", background: "transparent", border: "none", cursor: "pointer", padding: "4px" }} className="text-muted-foreground hover:text-foreground transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="flex gap-1.5 px-6 pt-4">
            {welcomeSlides.map((_, i) => (
              <button key={i} onClick={() => goTo(i, i > slide ? "right" : "left")} style={{ height: "3px", flex: 1, background: i === slide ? "hsl(var(--primary))" : "hsl(var(--border))", border: "none", clipPath: "none", cursor: "pointer", transition: "background 0.2s", boxShadow: i === slide ? "0 0 6px rgba(225,29,72,0.5)" : "none" }} />
            ))}
          </div>

          <div className="px-6 py-5 min-h-[200px]" style={{ opacity: animating ? 0 : 1, transform: animating ? `translateX(${dir === "right" ? "20px" : "-20px"})` : "translateX(0)", transition: "opacity 0.18s ease, transform 0.18s ease" }}>
            {cur.body}
          </div>
