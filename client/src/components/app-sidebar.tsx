...
export function AppSidebar() {
  ...
  const sendFeedback = async () => {
    ...
    try {
      await fetch("/api/feedback", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: fbText, email: user?.email }) });
      setFbSent(true);
      setTimeout(() => { setFbOpen(false); setFbText(""); setFbSent(false); }, 1500);
    } catch {}
  };
  ...
  return (
    <>
      {fbOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.75)", backdropFilter: "blur(4px)" }} onClick={(e) => { if (e.target === e.currentTarget) setFbOpen(false); }}>
          <div className="w-full max-w-sm bg-card border border-border p-5 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setFbOpen(false)} className="text-muted-foreground hover:text-foreground text-xl leading-none">&times;</button>
            </div>
            {fbSent ? (
              <p className="text-green-400 text-sm font-mono text-center py-4">✅ Отправлено! Спасибо.</p>
            ) : (
              <>
                <textarea value={fbText} onChange={(e) => setFbText(e.target.value)} placeholder="Предложения, баги, идеи..." className="w-full h-28 bg-background border border-border text-sm p-3 resize-none focus:outline-none focus:border-primary" autoFocus />
                <button onClick={sendFeedback} disabled={!fbText.trim()} className="w-full py-2 bg-primary text-white font-display text-xs uppercase tracking-wider disabled:opacity-40">Отправить</button>
              </>
            )}
          </div>
        </div>
      ...
      <SidebarFooter className="border-t border-sidebar-border flex-shrink-0 p-3 space-y-2 overflow-hidden">
        ...
        {/* Обратная связь */}
        <button onClick={() => setFbOpen(true)} data-sidebar="true" className="flex items-center gap-2 w-full px-2 py-1.5 text-muted-foreground hover:text-primary border border-border hover:border-primary/40 transition-colors" style={{ clipPath: "none", background: "transparent", borderRadius: 0 }}>
