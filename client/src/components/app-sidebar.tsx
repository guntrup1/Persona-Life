...
function FeedbackModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: "Отправлено!", description: "Спасибо за обратную связь." });
        onClose();
      } else {
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            data-sidebar="true"
            style={{ clipPath: "none", background: "transparent", border: "none", cursor: "pointer", padding: "4px" }}
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-primary text-white text-xs font-display uppercase tracking-wider disabled:opacity-40 hover:brightness-110 transition-all"
          >
      ...
      <Sidebar>
                  ...
                  return (
                    <SidebarMenuItem key={item.title}>
                      <SidebarMenuButton asChild isActive={isActive} data-testid={item.testId}>
                        <Link href={item.url}>
                          <div
                            className="flex items-center gap-3 w-full px-3 py-2 transition-all duration-150 relative"
                            style={{
                          ...
                          >
                            {isActive && <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-white/80" />}
                            <item.icon className="w-4 h-4 flex-shrink-0" style={{ color: isActive ? "#ffffff" : undefined }} />
                            <span
                              className="font-display text-xs tracking-widest uppercase truncate font-bold"
                              style={{ color: isActive ? "#ffffff" : undefined }}
                            >
                              {item.title}
                            </span>
                            {isActive && <div className="ml-auto w-1 h-3 bg-white/60 flex-shrink-0" />}
                          </div>
                        </Link>
                      </SidebarMenuButton>
                    </SidebarMenuItem>
        ...
        <SidebarFooter className="border-t border-sidebar-border flex-shrink-0 p-3 space-y-2 overflow-hidden">
          ...
          <button
            ...
            data-sidebar="true"
            className="flex items-center gap-2 w-full px-2.5 py-1.5 text-muted-foreground hover:text-primary border border-border hover:border-primary/50 transition-all duration-200"
            style={{ clipPath: "none", background: "transparent", borderRadius: 0, cursor: "pointer" }}
          >
